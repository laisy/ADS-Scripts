...
# Obter o nome do processo que será monitorado
  ...
  echo "PID  %CPU  %MEM  TIME MSECOND" > resultado_processo.txt 
  echo "PID  %CPU  %MEM  TIME MSECOND"
...
do
  ...
  if [ -n "$pid" ]; then
    ...
    echo "$pid $process_info $elapsed_time" >> resultado_processo.txt
    echo "$pid $process_info $elapsed_time"
