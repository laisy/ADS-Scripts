...
# Obter o nome do processo que será monitorado
  ...
  echo "DATA_ATUAL HORA_ATUAL PID  MEMTOTAL %CPU %MEM TIME MSECOND" > resultado_processo_bash_memoria.txt 
  echo "DATA_ATUAL HORA_ATUAL PID  MEMTOTAL %CPU %MEM TIME MSECOND"

while true
do
  # Obter o ID do processo especificado
  pid=$(pgrep -f "/bin/bash ./monitoramento_geral_bash")
  ...
  if [ -n "$pid" ]; then
    ...
    echo "$data_atual $pid $memTotal $process_info $elapsed_time" >> resultado_processo_bash_memoria.txt
